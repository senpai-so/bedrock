...
fn minting_max_tokens() {
  ...
  let allowed2 = mock_info(MINTER, &[]);

  let _ = contract
    .execute(deps.as_mut(), mock_env(), allowed2, mint_msg2)
    .unwrap();

  let count = contract.num_tokens(deps.as_ref()).unwrap();
  assert_eq!(2, count.count);

  // Third mint should error out
  let mint_msg3 = ExecuteMsg::Mint(MintMsg::<Extension> {
    token_id: "petrify3".to_string(),
    owner: String::from("medusa3"),
    name: name.clone(),
    description: Some(description.clone()),
    image: None,
    extension: None,
  ...
  let allowed3 = mock_info(MINTER, &[]);

  let err = contract
    .execute(deps.as_mut(), mock_env(), allowed3, mint_msg3)
    .unwrap_err();

  assert_eq!(err, ContractError::MaxTokensExceeded {});
}
