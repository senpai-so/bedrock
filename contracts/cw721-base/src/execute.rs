...
use cosmwasm_std::{Addr, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, Storage};
  ...
  C: CustomMsg,
{
  ...
  pub fn execute(
    ...
    match msg {
      ...
      ExecuteMsg::Revoke { spender, token_id } => self.revoke(deps, env, info, spender, token_id),
  ...
  C: CustomMsg,
{
  pub fn mint(
    ...
    let max_token_count = contract_info.max_token_count;
    let cur_token_count = self.token_count(deps.storage)?;
